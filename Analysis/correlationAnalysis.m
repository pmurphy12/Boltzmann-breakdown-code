function [Stats,C1,C2,Cjoint,params] = correlationAnalysis(tStep,identificationString,varargin)
...
%     TITLE = ['Local Correlations N = ' num2str(N)];
    ...
    Files = Files(1:1000); % comment out to do full analysis
    ...
    C_cosine = zeros(length(Files),N^2/Fact);
...
%         testString2 = strcmp('params',shortList2);
    ...
    for i = 1:N/Fact
        for j = 1:N
            ...
            n = length(AngTemp);
            A = AngTemp*AngTemp';
            d_theta = pdist2(AngTemp,AngTemp);
            A(1:n+1:n^2) = NaN;
            if n ~= 0
                C1(ii,(j-1)*N/Fact+i) = length(ang1)/n;
                C2(ii,(j-1)*N/Fact+i) = length(ang2)/n;
                testCases = [ang(1)^2, ang(1)*ang(2), ang(2)^2];
                Cjoint(ii,(j-1)*N/Fact+i,1) = sum(A == testCases(1),'all')/n/(n-1);
                Cjoint(ii,(j-1)*N/Fact+i,2) = sum(A == testCases(2),'all')/n/(n-1)/2;
                Cjoint(ii,(j-1)*N/Fact+i,3) = sum(A == testCases(2),'all')/n/(n-1)/2;
                Cjoint(ii,(j-1)*N/Fact+i,4) = sum(A == testCases(3),'all')/n/(n-1);
                C_cosine(ii,(j-1)*N/Fact+i) = sum(cos(2*d_theta(setdiff(1:n^2,1:n+1:n^2))),'all')/n/(n-1);
%                 if length(AngTemp)>5
%                     5+1;
%                 end
%             elseif n == 1
...
%                 Cjoint(ii,(j-1)*N/Fact+i,2) = 0;
%                 Cjoint(ii,(j-1)*N/Fact+i,3) = 0;
%                 Cjoint(ii,(j-1)*N/Fact+i,4) = sum(A == testCases(3),'all');
    ...
    Stats.cosine_corr = reshape(mean(C_cosine,1,'omitnan'),[N/Fact,N]);
...
%     Stats.fixedSpacePvals = reshape(diag(Stats.pvals),[N,N]);
    ...
    if SAVEDATA
        save(['Statistical conditional correlations and marginals for time step ' num2str(tStep) ' and spatial discretization ' num2str(N)],'Stats','C1_num','C2_num','Cjoint_num','C_denom')
    end

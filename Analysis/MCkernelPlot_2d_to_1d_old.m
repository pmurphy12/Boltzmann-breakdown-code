function [PDF,PDF_1d,X,Y] = MCkernelPlot_2d_to_1d(data,params,varargin)
...
%     dt = params.dt;
    ...
    PDF = cell(2,length(data));
    PDF_1d = cell(2,length(data));
...
%         tCurr = data{i}.t;
        ...
        [~,PDF{1,i},X,Y] = kde2d(pos1,N,[0 0],[Ly Lx],bwd(1),bwd(2));
        [~,PDF{2,i},~,~] = kde2d(pos2,N,[0 0],[Ly Lx],bwd(1),bwd(2));
        PDF{1,i} = PDF{1,i}*N^2/2;
        PDF{2,i} = PDF{2,i}*N^2/2;
        
%         PDF_1d{1,i} = trapz(Y(:,1),PDF{1,i},1)/Ly;
%         PDF_1d{2,i} = trapz(Y(:,1),PDF{2,i},1)/Ly;
        
        PDF_1d{1,i} = ksdensity(pos1(:,1),X(1,:));
        PDF_1d{2,i} = ksdensity(pos2(:,1),X(1,:));


    end
    p.done() % Carrage return for progress bar display in Command Window.
    
    save([save_name simType ' after kde2d ' DATE],'PDF','PDF_1d','X','Y')
    ...
    for i=1:size(PDF,2)
        mesh(X,Y,PDF{1,i})
        axis([0 Lx 0 Ly 0 max(PDF{1,1},[],'all')])
        xlabel('x')
        ylabel('y')
        zlabel('Density')
        M = getframe(gcf);
    ...
    for i=1:size(PDF,2)
        mesh(X,Y,PDF{2,i})
        axis([0 Lx 0 Ly 0 max(PDF{2,1},[],'all')])
        xlabel('x')
        ylabel('y')
        zlabel('Density')
        M = getframe(gcf);
    ...
    %% 1D movies
    v = VideoWriter([save_name simType ' 1D after kde2d ' DATE ' rho_1 1d video 75pi-2']);
    v.FrameRate = 5;
    open(v)
    ...
    for i=1:size(PDF_1d,2)
        plot(X(1,:),PDF_1d{1,i})
        axis([0 Lx 0 max(PDF_1d{1,1},[],'all')])
        xlabel('x')
        ylabel('Density')
        M = getframe(gcf);
        writeVideo(v,M)
    ...
    v = VideoWriter([save_name simType ' 1D after kde2d ' DATE ' rho_2 1d video 25pi-2']);
    v.FrameRate = 5;
    open(v)
    ...
    for i=1:size(PDF_1d,2)
        plot(X(1,:),PDF_1d{2,i})
        axis([0 Lx 0 max(PDF_1d{2,1},[],'all')])
        xlabel('x')
        ylabel('Density')
        M = getframe(gcf);
        writeVideo(v,M)

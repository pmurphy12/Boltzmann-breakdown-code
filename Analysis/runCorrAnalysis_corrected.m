function [time_stats] = runCorrAnalysis_corrected(tSteps,identificationString,varargin)
...
% resulting time-course data.
    ...
    for i = 1:length(tSteps)
        i
        % get spatial data
        [Stats,~,~,~,params] = correlationAnalysis_corrected_nonconditional2(tSteps(i),identificationString,'N',N,'Fact',Fact,'SAVEFIG',false);
        
        % Calculate averages over spatial discretization
        ...
        time_stats.tSteps = tSteps;
    end
    
    save(['Statistical pairs independence test with discretization ' num2str(N)],'time_stats')

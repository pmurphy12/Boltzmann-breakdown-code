function [Stats,C,Cjoint] = correlationAnalysisBinsN(tStep,identificationString,varargin)
...
%     TITLE = ['Local Correlations N = ' num2str(N)];
    TITLE = ['Numbered Local Independence Ratio N = ' num2str(N)];
    TITLE2 = ['Numbered Local Independence N = ' num2str(N)];
                ...
                for jo = 1:nBins
                    if io == jo
                        if  ~isempty(angi) %Added in normalization based on conditional expectation!!!!!
                            Cjoint(1,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = angi^2;
                        else
                        ...
                        if  ~isempty(angi) && ~isempty(angj)
                            Cjoint(1,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = angi*angj;
                        else
                            Cjoint(1,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = NaN;
                        end                      
                    end
            ...
            for j = 1:N
                ...
                for io = 1:nBins
                    ...
                    for jo = 1:nBins
                        if io == jo         
                            if  ~isempty(angi) %Added in normalization based on conditional expectation!!!!!
                                Cjoint(ii,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = angi^2;
                            else
                            ...
                            angj = sum(I & Ij);
                            if  ~isempty(angi) && ~isempty(angj)
                                Cjoint(ii,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = angi*angj;
                            else
                                Cjoint(ii,(j-1)*N/Fact+i,(jo-1)*(nBins)+io) = NaN;
                            end                      
                        end
...
%     Stats.fixedSpacePvals = reshape(diag(Stats.pvals),[N,N]);
    ...
    save(['Numbered Statistical correlations and marginals for time step ' num2str(tStep) ', spatial discretization ' num2str(N) ', and orientation discretization ' num2str(nBins)],'Stats','C','Cjoint')
    
    if PLOT
        [X,Y] = meshgrid(x(1:end-1),y(1:end-1));
        h = figure('Position',[300 100 300+2*30 300+2*300]); 
%         surf(X,Y,Stats.fixedSpaceCorr');
        surf(X,Y,Stats.independenceRatio(:,:,3)');
        view([0 90])
        colorbar
        caxis([-N N])
        xlabel('x')
        ylabel('y')
        title([TITLE ' Timestep ' num2str(tStep)  ' nAngBins ' num2str(nBins)])
        
        if SAVE
            savefig([TITLE ' 3 Timestep ' num2str(tStep) ' nAngBins ' num2str(nBins)])
            saveas(h,[TITLE ' 3 Timestep ' num2str(tStep)  ' nAngBins ' num2str(nBins) '.png'],'png')
        end
    end
    
    if PLOT
        [X,Y] = meshgrid(x(1:end-1),y(1:end-1));
        h2 = figure('Position',[300 100 300+2*30 300+2*300]); 
%         surf(X,Y,Stats.fixedSpaceCorr');
        ...
        xlabel('x')
        ylabel('y')
        title([TITLE2 ' Timestep ' num2str(tStep)  ' nAngBins ' num2str(nBins)])
        
        if SAVE
            savefig([TITLE2 ' 3 Timestep ' num2str(tStep) ' nAngBins ' num2str(nBins)])
            saveas(h2,[TITLE2 ' 3 Timestep ' num2str(tStep)  ' nAngBins ' num2str(nBins) '.png'],'png')
        end
    end

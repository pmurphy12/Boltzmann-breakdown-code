function [Stats,C1_num,C2_num,Cjoint_num,params] = correlationAnalysis_corrected_nonconditional(tStep,identificationString,varargin)
...
%     C1_num_alt = C1_num./params.N./C_denom_pairs;
%     C2_num_alt = C2_num./params.N./C_denom_pairs;
%     C1_num_alt(abs(C1_num_alt) == Inf) = NaN;
%     C2_num_alt(abs(C2_num_alt) == Inf) = NaN;
%     
...
%     Stats.N1N2joint(:,4) = Stats.N1N2joint(:,4)./Stats.denom_pairs';
    ...
    Stats.N1Marginal_alt = mean(C1_num_alt,1,'omitnan');%./Stats.denom;
    Stats.N2Marginal_alt = mean(C2_num_alt,1,'omitnan');%./Stats.denom;

    
    
    % calculate probabilities of rod (or pair) being in subregion ij
    Stats.denom = mean(C_denom,1,'omitnan');%/params.N;
    Stats.mean_noreplace = mean(C_denom-1,1,'omitnan');%/(params.N-1);
    Stats.denom_pairs = mean(C_denom_pairs,1,'omitnan');%/params.N/(params.N-1);
    Stats.denom_pairs_comp = Stats.denom.*Stats.mean_noreplace;
    ...
    Stats.independence11 = reshape(Stats.N1N2joint(:,1)-(Stats.N1Marginal.^2.*Stats.num_num_pair_ratio)',[N/Fact,N]);
    Stats.independence12 = reshape(Stats.N1N2joint(:,2)-(Stats.N1Marginal.*Stats.N2Marginal.*Stats.num_num_pair_ratio)',[N/Fact,N]);
    Stats.independence21 = reshape(Stats.N1N2joint(:,3)-(Stats.N1Marginal.*Stats.N2Marginal.*Stats.num_num_pair_ratio)',[N/Fact,N]);
    Stats.independence22 = reshape(Stats.N1N2joint(:,4)-(Stats.N2Marginal.^2.*Stats.num_num_pair_ratio)',[N/Fact,N]);
    
    Stats.independenceRatio11 = reshape((Stats.N1N2joint(:,1)-(Stats.N1Marginal.^2.*Stats.num_num_pair_ratio)')./(Stats.N1Marginal.^2'),[N/Fact,N]);
    Stats.independenceRatio12 = reshape((Stats.N1N2joint(:,2)-(Stats.N1Marginal.*Stats.N2Marginal.*Stats.num_num_pair_ratio)')./(Stats.N1Marginal'.*Stats.N2Marginal'),[N/Fact,N]);
    Stats.independenceRatio21 = reshape((Stats.N1N2joint(:,3)-(Stats.N1Marginal.*Stats.N2Marginal.*Stats.num_num_pair_ratio)')./(Stats.N1Marginal'.*Stats.N2Marginal'),[N/Fact,N]);
    Stats.independenceRatio22 = reshape((Stats.N1N2joint(:,4)-(Stats.N2Marginal.^2.*Stats.num_num_pair_ratio)')./(Stats.N2Marginal.^2'),[N/Fact,N]);
    
    Stats.independence11_alt = reshape(Stats.N1N2joint(:,1)-(Stats.N1Marginal_alt.^2)',[N/Fact,N]);
    Stats.independence12_alt = reshape(Stats.N1N2joint(:,2)-(Stats.N1Marginal_alt.*Stats.N2Marginal_alt)',[N/Fact,N]);
    Stats.independence21_alt = reshape(Stats.N1N2joint(:,3)-(Stats.N1Marginal_alt.*Stats.N2Marginal_alt)',[N/Fact,N]);
    Stats.independence22_alt = reshape(Stats.N1N2joint(:,4)-(Stats.N2Marginal_alt.^2)',[N/Fact,N]);

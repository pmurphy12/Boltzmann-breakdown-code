function [time_stats] = runCorrAnalysis_pooled(tSteps,identificationString,varargin)
...
% resulting time-course data.
    ...
    for i = 1:length(tSteps)
        i
        [Stats,~,~,~,params] = correlationAnalysis_pooled(tSteps(i),identificationString,'N',N,'Fact',Fact,'SAVEFIG',false);
...
%         time_stats.meanCosineCorr(i) = mean(Stats.cosine_corr(~isinf(Stats.cosine_corr)),'all','omitnan');
    ...
    save(['pooled conditional statistical independence test with discretization ' num2str(N)],'time_stats')

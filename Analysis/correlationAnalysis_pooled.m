function [Stats,C1_num,C2_num,Cjoint_num,params] = correlationAnalysis_pooled(tStep,identificationString,varargin)
...
%         testString2 = strcmp('params',shortList2);
    ...
    % pooled statistics
    Stats.N1Marginal = sum(C1_num,1,'omitnan')./sum(C_denom,1,'omitnan');
    Stats.N2Marginal = sum(C2_num,1,'omitnan')./sum(C_denom,1,'omitnan');
    temp = squeeze(sum(Cjoint_num,1,'omitnan'));
    temp2 = sum(C_denom_pairs,1,'omitnan')';
    Stats.N1N2joint(:,1) = temp(:,1)./temp2;
    Stats.N1N2joint(:,2) = temp(:,2)./temp2;
    Stats.N1N2joint(:,3) = temp(:,3)./temp2;
    Stats.N1N2joint(:,4) = temp(:,4)./temp2;
    ...
    Stats.correlation = (ang(1)^2*Stats.N1N2joint(:,1)+ang(1)*ang(2)*Stats.N1N2joint(:,2)...
                        +ang(2)*ang(1)*Stats.N1N2joint(:,3)+ang(2)^2*Stats.N1N2joint(:,4)...
                        -Stats.m.^2)...
...
%     [Stats.correlation,Stats.pvals] = corr(C1_num,C2_2);
